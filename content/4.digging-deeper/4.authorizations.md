...
| Policy method | Used when |
|---------------|-----------|
| `viewAny` | Listing / searching resources |
| `view` | Reading a specific model |
| `create` | Creating a new model |
| `update` | Updating an existing model |
| `replicate` | Duplicating a model via mutate |
| `delete` | Deleting a model |
| `restore` | Restoring a soft-deleted model |
| `forceDelete` | Force-deleting a model |
...
{
    /**
     * Determine for how much time the cache should be kept.
     *
