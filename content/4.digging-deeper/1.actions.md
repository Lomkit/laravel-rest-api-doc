...
By default, a classic action impacts **every** model of the resource when no `search` is provided, and there is no way to force the caller to narrow that perimeter. When you want to guarantee that an action only runs against models the caller explicitly designates, register it as `restricted` by invoking the `restricted` method.
...
class UserResource extends Resource
{
    ...
    public function actions(RestRequest $request): array {
        return [
            DeactivateUsersAction::make()->restricted()
        ];
    }
}
```

A restricted action requires the caller to provide a `resources` key (an array of ids) naming exactly which models to act on. The `search` operation stays optional and intersects with those ids, so a restricted action can never impact all models. See the [actions endpoint](/endpoints/actions) for the request payload.

`restricted` is mutually exclusive with `standalone`: an action cannot be both.
