...
| `includes.alias`      | `string` |                                      |             | A custom key for the relation in the response. Lets you include the same relation several times                |
...
You may optionally define an `alias` for an include. The relation is then returned under that key in the response instead of its default name.

An alias also lets you include the **same relation several times** with different arguments, each result being exposed under its own key:
  ...
  "search": {
    "includes": [
      ...
      },
      {
        ...
        "alias": "draftPosts",
        "filters": [
          {"field": "is_published", "value": false}
        ]
      }
    ]
  }
...
The alias is used verbatim as the response key. If two includes at the same level resolve to the same key, the last one wins.
