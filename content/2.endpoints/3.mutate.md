...
| `key`               | `mixed`   | When operation is `attach`, `detach`, `update`, `toggle` or `sync` |             | The model identifier or an array of model identifiers |
...
You can pass an empty array as parameter if you want to detach all records related to the parent model.

Example to detach all related records:
```json
// (POST) api/users/mutate
{
    "mutate": [
